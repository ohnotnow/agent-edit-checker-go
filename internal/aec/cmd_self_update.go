...
// selfUpdate handles aec self-update. It downloads the latest release
...
func selfUpdate(args []string, stdin io.Reader, stdout, stderr io.Writer) error {
	...
	if err := fs.Parse(args); err != nil {
		if errors.Is(err, flag.ErrHelp) {
			fmt.Fprint(stdout, selfUpdateUsage)
			return nil
		}
		return usageErr("self-update: %v", err)
	}
...
// emDash is the separator the update messages use, written as a rune so
// the source stays ASCII.
var emDash = string(rune(0x2014))

const selfUpdateUsage = `usage: aec self-update [--check] [--yes]
...
Exit codes for --check:
  ...
  2   Could not look it up (network etc.)
...
func runSelfUpdate(cfg selfUpdateConfig) error {
	...
	if cfg.currentVersion == "dev" {
		if cfg.checkOnly {
			fmt.Fprintln(cfg.stdout, "aec is a dev build "+emDash+" version check skipped.")
			return nil
		}
		fmt.Fprintln(cfg.stdout, "aec is a dev build "+emDash+" self-update is disabled.")
	...
	if err != nil {
		...
		if cfg.checkOnly {
			return &cliError{code: 2, msg: wrapped.Error()}
		}
		return wrapped
	}
	...
	if cfg.checkOnly {
		fmt.Fprintf(cfg.stdout, "aec %s is installed; %s is available.\n",
	...
	if pmAct == pmRedirect {
		fmt.Fprintf(cfg.stdout, "aec was installed via %s.\n", pmHint.manager)
		fmt.Fprintf(cfg.stdout, "A newer version (%s) is available "+emDash+" update with:\n  %s\n", rel.TagName, pmHint.command)
		return nil
	}

	assetName := assetNameFor(cfg.osName, cfg.arch)
	if assetName == "" {
		return validationErr("no release asset for platform %s/%s", cfg.osName, cfg.arch)
	}

	binAsset, ok := findAsset(rel.Assets, assetName)
	if !ok {
		return notFoundErr("release %s does not include asset %q", rel.TagName, assetName)
	}
	sumsAsset, ok := findAsset(rel.Assets, "SHA256SUMS")
	if !ok {
		return notFoundErr("release %s does not include SHA256SUMS", rel.TagName)
	}
	...
	if !canWrite(dir) {
		return validationErr(
			"cannot write to %s "+emDash+" re-run with sudo, or visit %s/releases/latest",
			dir, RepoURL)
	}
	...
	if !strings.EqualFold(actual, expected) {
		return validationErr("checksum mismatch for %s: expected %s, got %s", assetName, expected, actual)
	}

	if pmAct == pmWarn && cfg.stderr != nil {
		fmt.Fprintf(cfg.stderr,
			"warning: aec lives in %s, which is normally managed by your system package manager.\n",
			filepath.Dir(cfg.targetPath))
		fmt.Fprintln(cfg.stderr, "self-update will sidestep that "+emDash+" proceeding anyway.")
	}
...
func confirmUpdate(cfg selfUpdateConfig, rel *ghRelease, assetName string) (bool, error) {
	fmt.Fprintf(cfg.stdout, "aec %s -> %s\n", cfg.currentVersion, rel.TagName)
...
func detectPackageManager(path, goos, gopath, home string) (pmHint, pmAction) {
	...
	if strings.HasPrefix(p, "/opt/homebrew/") ||
		...
		return pmHint{manager: "Homebrew", command: "brew upgrade aec"}, pmRedirect
	}
	...
	if goBin != "" && (p == goBin || strings.HasPrefix(p, goBin+"/")) {
		return pmHint{
			manager: "'go install'",
			command: "go install github.com/ohnotnow/agent-edit-checker-go/cmd/aec@latest",
		}, pmRedirect
	}
...
func canWrite(dir string) bool {
	f, err := os.CreateTemp(dir, ".aec-write-test-*")
	if err != nil {
		return false
	}
	name := f.Name()
...
// assetNameFor returns the release asset filename the release workflow publishes for a given
...
func assetNameFor(goos, goarch string) string {
	...
	case "windows":
		if goarch == "amd64" || goarch == "arm64" {
			return "aec-windows-" + goarch + ".exe"
		}
	case "darwin":
		if goarch == "amd64" || goarch == "arm64" {
			return "aec-darwin-" + goarch
		}
	case "linux":
		if goarch == "amd64" || goarch == "arm64" {
			return "aec-linux-" + goarch
		}
	}
	return ""
}
...
func lookupChecksum(sums, name string) (string, error) {
	...
	return "", notFoundErr("no checksum entry for %q in SHA256SUMS", name)
}
